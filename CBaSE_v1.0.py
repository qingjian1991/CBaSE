...
def neg_ln_L(p, genes, aux):
	...
	if modC==1:
		...
		for gind in range(len(genes)):
			...
			#*************** lambda ~ IG:
			if s>25:
				summe += math.log(2.) + ((s + a)/2.)*math.log(b) + mp.log(mp.besselk(-s + a, 2*math.sqrt(b))) - sp.gammaln(s+1) - sp.gammaln(a)
			else:
				summe += math.log(2.) + ((s + a)/2.)*math.log(b) + math.log(sp.kv(-s + a, 2*math.sqrt(b))) - sp.gammaln(s+1) - sp.gammaln(a)
	elif modC==3:
		for gind in range(len(genes)):
			s = int(genes[gind]["obs"][2])
			#*************** lambda ~ w * Exp + (1-w) * Gamma:
			summe += np.log( math.exp( math.log(w * t) + (-1 - s)*math.log(1 + t) ) + math.exp( math.log(1.-w) + s*math.log(b) + (-s-a)*math.log(1 + b) + sp.gammaln(s + a) - sp.gammaln(s+1) - sp.gammaln(a) ) )
	elif modC==4:
		for gind in range(len(genes)):
			s = int(genes[gind]["obs"][2])
			#*************** lambda ~ w * Exp + (1-w) * InvGamma:
			if s>25:
				summe += np.log( math.exp( math.log(w * t) + (-1 - s)*math.log(1 + t) ) + math.exp( math.log(1.-w) + math.log(2.) + ((s + a)/2.)*math.log(b) + mp.log(mp.besselk(-s + a, 2*math.sqrt(b))) - sp.gammaln(s + 1) - sp.gammaln(a) ) )
			else:
				summe += np.log( math.exp( math.log(w * t) + (-1 - s)*math.log(1 + t) ) + math.exp( math.log(1.-w) + math.log(2.) + ((s + a)/2.)*math.log(b) + math.log(sp.kv(-s + a, 2*math.sqrt(b))) - sp.gammaln(s + 1) - sp.gammaln(a) ) )
	elif modC==5:
		for gind in range(len(genes)):
			s = int(genes[gind]["obs"][2])
			#*************** lambda ~ w * Gamma + (1-w) * Gamma:
			summe += np.log( np.exp( np.log(w) + s*np.log(b) + (-s-a)*np.log(1 + b) + sp.gammaln(s + a) - sp.gammaln(s+1) - sp.gammaln(a) ) + np.exp( np.log(1.-w) + s*np.log(d) + (-s-g)*np.log(1 + d) + sp.gammaln(s + g) - sp.gammaln(s+1) - sp.gammaln(g) ) )
	elif modC==6:
		for gind in range(len(genes)):
			s = int(genes[gind]["obs"][2])
			#*************** lambda ~ w * Gamma + (1-w) * InvGamma:
			if s>25:
				summe += np.log( (w * math.exp(s*math.log(b) + (-s-a)*math.log(1 + b) + sp.gammaln(s + a) - sp.gammaln(s+1) - sp.gammaln(a))) + ((1.-w) * math.exp( math.log(2.) + ((s + g)/2.)*math.log(d) + mp.log(mp.besselk(-s + g, 2*math.sqrt(d))) - sp.gammaln(s + 1) - sp.gammaln(g) ) ))
			else:
				summe += np.log( (w * b**s * (1 + b)**(-s-a) * math.exp(sp.gammaln(s + a) - sp.gammaln(s+1) - sp.gammaln(a))) + ((1.-w) * math.exp( math.log(2.) + ((s + g)/2.)*math.log(d) + math.log(sp.kv(-s + g, 2*math.sqrt(d))) - sp.gammaln(s + 1) - sp.gammaln(g) ) ))

	lastres = -summe
	if lastres>1e8:
		sys.stderr.write("-ln(L) too large. Run again or try changing s threshold in neg_ln_L() or omitting genes with large s.\n")
		return 0
	return -summe
...
def compute_p_values(p, genes, aux):
	...
	for gene in genes:
		...
		for rep in range(runC):
			...
			pvals.append([gene["gene"], m_pneg, k_pneg, m_ppos, k_ppos, pofx_given_s(0, sobs, L, ratm, 0), pofx_given_s(0, sobs, L, ratk, 0), mobs, kobs, sobs])
	...
	for gene in sorted_phi:
		if gene["phi"] > phi_sim_max+0.5*bin_phi:
			...
			if abs(gene["phi"])<1e-30:
				phi_pvals_obs.append({"gene": gene["gene"], "p_phi": 1.-cumprob, "phi": gene["p0m"]*gene["p0k"], "mks": gene["mks"]})
			else:
				cumprob=0.; i=0
...
neutral_mutations = [mut for mut in mutations if ["missense", "nonsense", "coding-synon", "utr-3", "utr-5"].count(mut["muttype"]) and cancer_genes.count(mut["gene"])==0 and essential_genes.count(mut["gene"])==0]
...
mks_type=[]
for gene in res:
	mks_type.append({"gene": gene[0], "exp": [float(gene[1]), float(gene[2]), float(gene[3])], "obs": [float(gene[4]), float(gene[5]), float(gene[6])], "len": int(gene[7])})
...
sys.stderr.write("Filtered out genes with M+K+S=0 in TumorPortal pan-cancer analysis, leaving %i genes.\n" %len(mks_type))

fout = open("output_data_preparation.txt", "w")
# Output format: [gene, lm, lk, ls, mobs, kobs, sobs, Lgene]
for gene in mks_type:
	fout.write("%s\t%f\t%f\t%f\t%i\t%i\t%i\t%i\n" %(gene["gene"], gene["exp"][0], gene["exp"][1], gene["exp"][2], gene["obs"][0], gene["obs"][1], gene["obs"][2], gene["len"]))
fout.close()
